...
class Logger(ProgressMsg):
    def __init__(self,
                 max_iter: tuple = None,
                 log_dir: str = None,
                 log_file_option: str = 'w',
                 log_lvl: str = 'note',
                 log_file_lvl: str = 'info',
        ...
        """Args:
            ...
            write_log (bool) : whether to write logs to file
        """
        ...
        if self.write_log and self.log_dir is not None:
            logfile_time = datetime.datetime.now().strftime('%m-%d-%H')
            self.log_file = open(os.path.join(log_dir, f"log_{logfile_time}.log"), log_file_option)
            self.val_file = open(f"{log_dir}/validation_{logfile_time}.log", log_file_option)
    ...
    def _print(self, txt, lvl_n, end):
        ...
        if self.write_log and self.log_file_lvl <= lvl_n:
            self.write_file(txt)
